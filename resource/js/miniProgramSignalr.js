...
export class HubConnection {
  ...
  start(url, queryString) {
    ...
    wx.request({
      ...
      success: res => {
        ...
        console.log(this.negotiateResponse)
        this.startSocket(negotiateUrl.replace("/negotiate","/connect"));
      },
      fail: res => {
  ...
  startSocket(url) {
    url += (url.indexOf("?") < 0 ? "?" : "&") + ("transport=webSockets&connectionToken=" + encodeURIComponent(this.negotiateResponse.ConnectionToken) + "&connectionData=" + encodeURIComponent('[{ "name": "sorunmesshub" }]'));
  ...
  receive(data) {
    ...
    switch (message.S) {
      ...
      case MessageType.Close:
        ...
        break;
      default:
        console.warn("Invalid message type: " + message.S);
    }
